...
import {
  ...
  uploadQuestionBank,
} from "../../services/questionBankApi.js";
...
function QuestionBankPage({
  ...
  const fetchQuestionBank = async () => {
    ...
    try {
      const data = await getQuestionBank(job.id);
  ...
  const uploadFile =
    async () => {
      ...
      try {
        ...
        const data = await uploadQuestionBank(job.id, file);
  ...
  const filteredSavedQuestions = filterQuestionBank(savedQuestions, {
  ...
  const saveEditedQuestion = async () => {
    ...
    try {
      await updateQuestionApi(editingQuestionId, {
        job_id: job.id,
        ...editingQuestion,
  ...
  const deleteQuestion = async (question) => {
    ...
    try {
      await deleteQuestionApi(questionId, job.id);
  ...
  const clearQuestionBank = async () => {
    ...
    try {
      await clearQuestionBankApi(job.id);
