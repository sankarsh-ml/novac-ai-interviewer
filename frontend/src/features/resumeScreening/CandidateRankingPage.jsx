...
import CandidateStats from "../../components/candidates/CandidateStats.jsx";
...
import { getResumeDownloadUrl } from "../../services/resumeApi.js";
...
function JobApplicationsPage({job,onBack,onViewShortlisted,onViewResume}) {
  ...
  const deleteApplication = async (applicationId) => {
    ...
    try {
      await deleteCandidate(applicationId);
      fetchApplications();
    } catch (error) {
      console.error(error);
  ...
  const updateHRDecision = async (applicationId, decision) => {
      try {
        await updateCandidateDecision(applicationId, decision);
        fetchApplications();
      } catch (error) {
        console.error(error);
  ...
  const quickSelectCandidates = async (event) => {
    ...
    try {
      const data = await runQuickSelectCandidates(job.id, count);
  ...
  const deleteAllRecords = async () => {
    ...
    try {
      const data = await deleteAllCandidateRecords(job.id);
      setApplications([]);
                  ...
                  <td>
                    <div className="resume-actions">
                      <button
                        ...
                        onClick={() =>
                          window.open(
                            getResumeDownloadUrl(app.application_id),
                            "_blank"
                          )
                        }
                      >
