...
import * as interviewUseCases from "@application/useCases/interviewUseCases.js";
...
function InterviewPage({ applicationSummary, cameraSession, onBackHome }) {
  ...
  const startInterview = (...args) => interviewUseCases.startInterview(interviewRepository, ...args);
  const transcribeInterviewAudio = (...args) => interviewUseCases.transcribeInterviewAudio(interviewRepository, ...args);
  ...
  const handleStartInterview = async () => {
    ...
    try {
      ...
      if (
        ["complete", "completed"].includes(String(response.status || "").toLowerCase()) ||
        ["complete", "completed"].includes(String(response.interview_status || "").toLowerCase()) ||
        ["complete", "completed"].includes(String(response.interviewStatus || "").toLowerCase()) ||
...
function isInterviewCompleted(candidate) {
  return (
    ...
    ["complete", "completed"].includes(String(candidate?.interview_status || "").toLowerCase()) ||
    ["complete", "completed"].includes(String(candidate?.interviewStatus || "").toLowerCase())
  );
}
