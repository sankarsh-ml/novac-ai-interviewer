...
import {
  ...
  uploadQuestionBank,
} from "@application/useCases/questionBankUseCases.js";
import { useDependencies } from "@presentation/hooks/useDependencies.js";
import { downloadTextFile } from "@shared/utils/fileUtils.js";
import { formatDifficulty, normalizeDifficulty, splitTags } from "@domain/rules/questionRules.js";
import "@presentation/styles/QuestionBankPage.css";
...
function QuestionBankPage({
  ...
  const { questionBankRepository } = useDependencies();
  ...
  const saveQuestions =
    async () => {
      ...
      try {
        ...
        await saveQuestionBank(questionBankRepository, job.id, [
          ...savedQuestions,
          ...normalizedQuestions
        ]);
  ...
  const uploadFile =
    async () => {
      ...
      try {
        ...
        const data = await uploadQuestionBank(questionBankRepository, job.id, file);
  ...
  const filteredSavedQuestions = filterQuestionBank(questionBankRepository, savedQuestions, {
  ...
  const saveEditedQuestion = async () => {
    ...
    try {
      await updateQuestionApi(questionBankRepository, editingQuestionId, {
        job_id: job.id,
        ...editingQuestion,
  ...
  const deleteQuestion = async (question) => {
    ...
    try {
      await deleteQuestionApi(questionBankRepository, questionId, job.id);
  ...
  const clearQuestionBank = async () => {
    ...
    try {
      await clearQuestionBankApi(questionBankRepository, job.id);
